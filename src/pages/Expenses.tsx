    ...
    <div className="flex flex-col gap-4 pb-6 slide-up">
        ...
        <div>
          ...
          {filtered.length > 0 && (
            <p className="text-sm text-hint mt-0.5">
              {filtered.length} items · <span style={{ color: '#E74C3C' }}>{fmt(totalFiltered, defaultCurrency)}</span>
            </p>
          )}
        </div>
      ...
      <div className="px-5 flex gap-2">
        ...
        <button
          ...
          className="input-field rounded-xl px-3 flex items-center gap-1 shrink-0"
          style={{ color: showFilters ? 'var(--tg-theme-button-color)' : 'var(--tg-theme-hint-color)' }}
        >
      ...
      {filtered.length === 0 ? (
        <div className="mx-5 card flex flex-col items-center py-10 gap-3">
          ...
          <p className="text-sm text-hint text-center">No expenses yet.<br />Track your spending to get insights.</p>
          <button onClick={openAdd} className="btn-primary flex items-center gap-1">
            ...
            <div key={date}>
              <div className="flex items-center justify-between mb-2">
                ...
                <p className="text-xs text-hint">
                  {fmt(grouped[date].reduce((s, e) => s + e.amount, 0), defaultCurrency)}
                </p>
              </div>
                    ...
                    <div key={exp.id} className="card flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg active:opacity-60 shrink-0"
      ...
      <Sheet open={sheetOpen} onClose={() => setSheetOpen(false)} title="Add Expense">
        <div className="flex flex-col gap-3">
          ...
          <div>
            <label className="section-title block">Note (optional)</label>
            <input
              className="input-field"
          ...
          <button onClick={save} className="btn-primary w-full mt-2">Add Expense</button>
        </div>
      </Sheet>
    </div>
  );
...
function CategoryChip({ active, label, icon, onClick }: {
  ...
  return (
    <button
      ...
      style={{
        backgroundColor: active ? 'var(--tg-theme-button-color, #2481cc)' : 'var(--tg-theme-secondary-bg-color)',
        color: active ? 'var(--tg-theme-button-text-color, #fff)' : 'var(--tg-theme-text-color)',
      }}
    >
      {icon} {label}
    </button>
