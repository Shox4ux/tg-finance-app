...
export function Analysis() {
  ...
  const categoryData = useMemo(() => {
    ...
    return EXPENSE_CATEGORIES
      .map((cat) => ({ id: cat.id, name: catLabel(cat.id), value: map[cat.id] ?? 0, color: cat.color, icon: cat.icon }))
      .filter((d) => d.value > 0)
  ...
  return (
    <div className="flex flex-col gap-5 pb-8 slide-up w-full">
      <div className="px-5 sm:px-8 pt-5 sm:pt-7">
        <h2 className="text-2xl sm:text-3xl font-bold">{t('an_title')}</h2>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="mx-5 sm:mx-8 card flex flex-col items-center py-16 gap-3">
          <span className="text-5xl">📊</span>
          <p className="text-sm text-hint text-center" style={{ whiteSpace: 'pre-line' }}>{t('an_empty')}</p>
        </div>
      ) : (
        <>
          {/* Top stats - 4 cols on desktop */}
          <div className="px-5 sm:px-8 grid grid-cols-3 sm:grid-cols-4 gap-3 sm:gap-4">
            <div className="card text-center sm:py-5">
              <p className="text-xs text-hint mb-1">{t('an_total')}</p>
              <p className="font-bold sm:text-lg" style={{ color: '#E74C3C' }}>{fmt(total, currency)}</p>
            </div>
            <div className="card text-center sm:py-5">
              <p className="text-xs text-hint mb-1">{t('an_avg')}</p>
              <p className="font-bold sm:text-lg">{fmt(total / Math.max(1, (period === '7d' ? 7 : period === '30d' ? 30 : period === '90d' ? 90 : filtered.length)), currency)}</p>
            </div>
            <div className="card text-center sm:py-5">
              <p className="text-xs text-hint mb-1">{t('an_savings_rate')}</p>
              <p className="font-bold sm:text-lg" style={{ color: savingsRate > 20 ? '#27AE60' : savingsRate > 0 ? '#E67E22' : '#E74C3C' }}>{savingsRate}%</p>
            </div>
            <div className="card text-center sm:py-5 col-span-3 sm:col-span-1">
              <p className="text-xs text-hint mb-1">{t('an_monthly_balance')}</p>
              <div className="flex justify-center gap-4">
                <div>
                  <p className="text-[10px] text-hint">{t('an_income')}</p>
                  <p className="font-bold text-sm" style={{ color: '#27AE60' }}>{fmt(monthlyIncome, currency)}</p>
                </div>
                <div>
                  <p className="text-[10px] text-hint">{t('an_expenses')}</p>
                  <p className="font-bold text-sm" style={{ color: '#E74C3C' }}>{fmt(monthlyExpenses, currency)}</p>
                </div>
              </div>
            </div>
          </div>
                      ...
                      return (
                        <div key={exp.id} className="card flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-hint">{catLabel(cat.id)}</p>
                          </div>
