    ...
    <div className="flex flex-col gap-4 pb-6 slide-up">
      <div className="px-5 pt-5 flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-hint mt-0.5">
            Monthly total: <span className="font-semibold" style={{ color: '#27AE60' }}>{fmt(monthlyIncome, defaultCurrency)}</span>
          </p>
        </div>
      ...
      {incomeSources.length === 0 ? (
        <div className="mx-5 card flex flex-col items-center py-10 gap-3">
          ...
          <p className="text-sm text-hint text-center">No income sources yet.<br />Add your salary, freelance, or any other income.</p>
          <button onClick={openAdd} className="btn-primary flex items-center gap-1">
      ...
      <Sheet open={sheetOpen} onClose={() => setSheetOpen(false)} title={editId ? 'Edit Income Source' : 'Add Income Source'}>
        <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              {(Object.keys(FREQ_LABELS) as IncomeFrequency[]).map((freq) => (
                <button
                  ...
                  style={{
                    backgroundColor: form.frequency === freq
                      ? 'var(--tg-theme-button-color, #2481cc)'
                      : 'var(--tg-theme-secondary-bg-color, #f1f1f1)',
                    color: form.frequency === freq
          ...
          <button onClick={save} className="btn-primary w-full mt-2">
            {editId ? 'Save Changes' : 'Add Income Source'}
          </button>
        </div>
      </Sheet>
    </div>
